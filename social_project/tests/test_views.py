...
class UserViewsTest(TestCase):
    ...
    def test_update_user_view(self):
        response = self.client.post(reverse('update_user', kwargs={'username': 'testuser'}), data=self.user_data)
        self.assertEqual(response.status_code, 302)
        updated_user = User.objects.get(username='newuser')
        self.assertEqual(updated_user.email, 'newuser@example.com')
    
    def test_delete_user_view(self):
        response = self.client.post(reverse('delete_user', kwargs={'username': 'testuser'}))
        self.assertEqual(response.status_code, 302)
...
class PostViewsTest(TestCase):
    ...
    def test_post_detail_view(self):
        post = Post.objects.create(user=self.user, post='this is a test post', visibility=Post.PUBLIC)
        response = self.client.get(reverse('post_detail', kwargs={'post_id': post.id}))
        self.assertEqual(response.status_code, 200)
        #self.assertContains(response, 'this is a test post')
        
    def test_update_post_view(self):
        post = Post.objects.create(user=self.user, post='this isOld post', visibility=Post.PUBLIC)
    ...
    def test_delete_post_view(self):
        post = Post.objects.create(user=self.user, post='To be deleted', visibility=Post.PUBLIC)
        response = self.client.post(reverse('delete_post', kwargs={'username': 'testuser', 'post_id': post.id}))
        self.assertEqual(response.status_code, 302)
